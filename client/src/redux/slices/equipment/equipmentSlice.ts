...
import { addEquipThunk, getAllEquipmentThunk, getOneEquipThunk } from './equipmentThunkActions';
...
const equipmentSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(addEquipThunk.fulfilled, (state, action) => {
      state.equipment = [action.payload, ...state.equipment];
    });

    builder.addCase(getOneEquipThunk.fulfilled, (state, action) => {
      state.oneEquipment = action.payload;
    });
  },
});

export default equipmentSlice.reducer;
export const { setCleanEquip, setSelectedCategoryEquipment, changeCategoryEquipment } =
